...
3. Use Github.  Using github will keep you up-to-date with the developers and will allow you to contribute to the project.  We do not store the z80 libraries or the binaries in the github repository.  Instead you will either have to build those things yourself or acquire them from the nightly build to have a working install.
	1. **Installing the C development environment** The system is built with C-98 and C-11 and depends on [GLib-2.0](https://developer.gnome.org/glib/stable/ "GLib-2.0"). On a POSIX system, pkg-config is used to find out the location of the GLib libraries.
   2. **Installing the Z88DK Binaries**
	  ...
	  * **Windows** Download the nightly build for win32 and copy the z88dk/bin directory to the same place in your z88dk tree.  You can also build the z88dk binaries yourself using the VS2015 solution found in z88dk/win32 however you should copy the nightly build initially so that various required dlls and some non-z88dk binaries are present. On MinGW, pkg-config and glib can be installed in one go by running in the source pkg-config directory ```./configure --prefix=/mingw --with-internal-glib && make && make install```.  The Visual Studio solution file assumes GLib is installed in the default location in MinGW; the project is configured with:
		  * C/C++ / General / Additional Include Directories: ```C:/MinGW/include/glib-2.0;C:/MinGW/lib/glib-2.0/include```
		  * Linker / General / Additional Library Directories: ```C:/MinGW/lib```
		  * Linker / Input / Additional Dependencies: ```libglib-2.0.dll.a```   
	...
	4. **Installing the New Lib Z80 Libraries**  If you installed the z88dk binaries following the Other instructions in (I) you should have also built the new z80 libraries.  You can confirm this by checking that z88dk/libsrc/_DEVELOPMENT/lib contains six .lib files in each of the subdirectories.  Otherwise you can following one of these two methods:   
